...
def preprocess_df(df, text_col_candidate=None, use_pyvi=False):
    """
    ...
    # apply pyvi tokenization if requested and available
    if use_pyvi:
        if not USE_PYVI_AVAILABLE:
            print("PyVi không có sẵn — tiếp tục mà không tokenization PyVi.")
        else:
            print("Applying PyVi tokenization to text column...")
            df[text_col] = df[text_col].map(lambda t: ViTokenizer.tokenize(t) if isinstance(t, str) else t)
    return df, text_col
...
def fit_vectorizer(texts, output_dir, use_tfidf=True, max_features=30000):
    # texts expected to be pre-tokenized strings (tokens separated by spaces) when using PyVi
    if use_tfidf:
...
def fit_w2v(texts, output_dir, vector_size=800, window=10, min_count=1):
    # texts expected to be pre-tokenized strings -> split by whitespace
...
def train_br(train_df, val_df, text_col, aspect_cols, output_dir, random_state=42, use_pyvi=False):
    os.makedirs(output_dir, exist_ok=True)

    # When use_pyvi=True we assume train_df[text_col] already tokenized by preprocess_df
    vectorizer = fit_vectorizer(train_df[text_col].tolist(), output_dir, use_tfidf=False)
    w2v_model = None
    if USE_W2V:
        # fit w2v on tokenized text as well
    ...
    results = {}
    for aspect in tqdm(aspect_cols):
        ...
        pres_clf = LogisticRegression(max_iter=1000, solver='lbfgs', C= 0.3)
        ...
        if len(pos_idx) >= 5:
            ...
            sentiment_clf = LogisticRegression(max_iter=1500, solver='saga', multi_class='multinomial', C=0.3)
...
def main():
    parser = argparse.ArgumentParser(description='Train & Predict BR presence + sentiment (with optional PyVi tokenization)')
    ...
    parser.add_argument('--use_pyvi', action='store_true', help='If set, tokenize texts using PyVi before vectorizing/training')
    ...
    if args.mode == 'train':
        ...
        df, text_col = preprocess_df(df, use_pyvi=args.use_pyvi)
        ...
        results = train_br(train_df.reset_index(drop=True), val_df.reset_index(drop=True), text_col, aspect_cols, args.output_dir, use_pyvi=args.use_pyvi)
        ...
        test_df, text_col = preprocess_df(test_df, use_pyvi=args.use_pyvi)
